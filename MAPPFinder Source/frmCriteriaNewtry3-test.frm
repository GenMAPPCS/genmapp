...
Begin VB.Form frmCriteria 
   ...
   ClientTop       =   1500
   ClientWidth     =   6390
   Icon            =   "frmCriteriaNewtry3-test.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   ScaleHeight     =   8790
   ScaleWidth      =   6390
...
'The first time MAPPFinder is run on a .gex file, a .gmf file is created storing
...
Public Sub Load(FileName As String)
   ...
   Set rsSpecies = dbTemp.OpenRecordset("SELECT Species FROM INFO")
...
Private Sub cmdFile_Click()
   ...
   If invalidFileName(FileName) Or (InStr(1, FileName, ".") > 0) Then
      MsgBox "A filename cannot contain any of the following characters: /\:*?" & Chr(34) & "<>|. Also, periods outside the .txt extention are not allowed.", vbOKOnly
      txtFile.Text = ""
      Exit Sub
   End If
...
compact:
   If GOresultsExist Then
      dbChipData.Close
      'dbExpressionData.Close
      CompactDatabase newfilename & ".gmf", newfilename & ".$tm"
      Kill newfilename & ".gmf"
      Name newfilename & ".$tm" As newfilename & ".gmf"
   End If
   
   If LocalMAPPsLoaded Then
      dbChipDataLocal.Close
      DBEngine.CompactDatabase newfilename & "-Local.gmf", newfilename & "-Local.$tm"
      Kill newfilename & "-Local.gmf"
      Name newfilename & "-Local.$tm" As newfilename & "-Local.gmf"
   End If
   If GOresultsExist Or localmappresultsexist Then
...
error:
   Select Case Err.Number
      ...
      Case 3356
         MsgBox "You have the GenMAPP Expression Dataset " & newfilename & ".gex or .gmf file open in another window. You must close" _
...
Public Function mapToClusterSystem() As Boolean
   Dim rsSystem As Recordset, rsrelations As DAO.Recordset, rsMOD As DAO.Recordset
   ...
   Set rsSystem = dbMAPPfinder.OpenRecordset("SELECT SystemCode from Systems where" _
   ...
   If rsSystem.EOF = False Then
      ...
      Else 'check the other direction
         ...
         If rsRelation.EOF = False Then
            ...
            primary = True
         Else
            MsgBox "You do not have a table from " & clustersystem & " to GO, you need this. If you altered" _
            & " your database, you should re-download the original from GenMAPP.org. It is also possible that you" _
            & " are using the wrong species database, make sure you load the correct database at the start of MAPPFinder." _
            & " If you haven't changed " _
            & "anything in your database and have the correct species data loaded, then please contact genmapp@gladstone.ucsf.edu.", vbOKOnly
   ...
   If Dir(newfilename & ".gmf") = "" Then
      'build the mappings to clusterID and GO
      'else load the mappings from the gmf file
                     ...
                     If relations(i, 3) <> "" Then '___________________________________Check Secondary IDs
                        ...
                        If rsGeneID.EOF = False Then
                           ...
                           If rsrelations.EOF = False Then
                              addtoPrimaryGenes rsGenes!id, addtoclustergenes(rsGenes!id, rsGeneID!id)
                           Else
...
endwhile:
      ...
      lblProgress.Caption = "Storing the GO Mappings. This may take a few minutes..."
      frmCriteria.Refresh
      DoEvents
      
      If ClusterGenes.count = 0 Then
        MsgBox "No Gene Ontology results are available for this dataset. This is typically due to" _
            & " the fact that your gene ID system does not have a relationship" _
            & " to the MOD system (e.g. Refseq doesn't relate to Swiss-PROT in the human database). This relationship is needed for MAPPFinder.", vbOKOnly
        mapToClusterSystem = False
        Exit Function
      End If
      
      Set dbChipData = CreateDatabase(newfilename & ".gmf", dbLangGeneral)
   ...
   Else 'the mapping has been done before just read in the files.
      ...
      frmCriteria.lblProgress.Caption = "Loading GO mappings from " & newfilename & ".gmf"
      frmCriteria.Refresh
      DoEvents
   
      Set dbChipData = OpenDatabase(newfilename & ".gmf")
      Set rstemp = dbMAPPfinder.OpenRecordset("Select Version from INFO")
      ...
      If clustersystem <> rstemp2!gotable Then 'they are using the wrong database
         MsgBox "The gmf file, which stores the mapping of your expression data to GO, was built" _
               & " using " & rstemp2!gotable & ". Your current Gene Database suggests that " & clustersystem _
               & " should be used. Please check this discrepancy and either select a new database" _
               & " or delete your gmf file and start over."
         mapToClusterSystem = False
         frmStart.Show
         eraseForm
         Me.Hide
      ...
      ElseIf rstemp!Version <> rstemp2!Version Then
         'need to rebuild
         If MsgBox("The MAPPFinder gmf file storing the links from your Expression Dataset" _
            & " to Gene Ontology was built on a different version of the GenMAPP database." _
            & " To proceed using " & databaseloc & " MAPPFinder will need to rebuild the gmf" _
            ...
            Me.Hide
            Exit Function
            Else 'they want to rebuild the gmf file
         
            dbChipData.Close
            lblProgress.Caption = "Your gmf file is out of date. MAPPFinder is now going to rebuild it."
            frmCriteria.Refresh
            DoEvents
      ...
      Else 'they match load the data
         ...
         While rsGenes.EOF = False
            ...
            Else
               ...
               If (ClusterGenes.Item(rsGenes!related).getMasterPrimary = "") Then
                  'this is the first primary ID to hit this cluster gene. Add this
                  'and only this primary ID to the GO terms of this cluster gene.
                  
                  ClusterGenes.Item(rsGenes!related).setMasterPrimary rsGenes!primary
...
ClusterExists:
   NewCG.addLocalGOterm GOID, addtoLocalMAPPs(GOID, clusterID)
...
error:
   Select Case Err.Number
      Case 5
         ...
         cg.setMasterPrimary (primaryID) 'stores the first primary (only want to count it once)
         If primary Then
   ...
   Select Case Err.Number
      Case 5
         ...
         While rsMAPPs.EOF = False
            nohits = False
            cg.addLocalGOterm rsMAPPs!MAPP, addtoLocalMAPPs(rsMAPPs!MAPP, idIn, primaryID)
            rsMAPPs.MoveNext
         Wend
...
Public Function addtoLocalMAPPs(MAPPName As String, clusterID As String, Optional primaryID As String) As goterm
On Error GoTo error:
   Dim MAPP As goterm
...
resumefirst:
                     ...
                     parentterm.addGene (cg.getMasterPrimary())
...
resumehere:
         ...
         root.addGene (cg.getMasterPrimary())
...
Private Sub MaptoLocalMAPPs()
   ...
   rsGenes.MoveFirst
   GEXsize = rsGenes.RecordCount
   If Dir(newfilename & "-Local.gmf") = "" Then
      While rsGenes.EOF = False
         progress = progress + 1
      ...
      Set dbChipDataLocal = CreateDatabase(newfilename & "-Local.gmf", dbLangGeneral)
      ...
      tblMAPPtoGene.Fields.Append tblMAPPtoGene.CreateField("Related", dbText, IDSIZE)
      ...
      With tblnestedresults
         ...
         Dim idxNestedResults As Index
         Dim idxPercentage As Index
         .Fields.Append .CreateField("GOID", dbText, IDSIZE)
         Set idxPercentage = .CreateIndex("percentageindex")
         Set idxNestedResults = .CreateIndex("nestedResults")
         idxNestedResults.primary = True
         idxNestedResults.Fields.Append .CreateField("GOID", dbText, IDSIZE)
         idxPercentage.Fields.Append .CreateField("GOType", dbText, 2)
         idxPercentage.Fields.Append .CreateField("Percentage", dbSingle)
         idxPercentage.Fields.Append .CreateField("Zscore", dbSingle)
         .Indexes.Append idxNestedResults
   ...
   dbChipDataLocal.TableDefs.Append tblnestedresults
      ...
      For Each Gene In LocalPrimaryGenes
         'Debug.Print LocalPrimaryGenes.count
         Set relatedItems = Gene.getClusterGenes
         If relatedItems.count = 0 Then 'no cluster gene for this guy
      ...
      Next Gene
      DoEvents
      'Debug.Print LocalClusterGenes.count
   ...
   Else 'the mapping has been done before just read in the files.
      ...
      frmCriteria.lblProgress.Caption = "Loading LocalMAPP mappings from " & newfilename & ".gmf"
      frmCriteria.Refresh
      DoEvents
   
      Set dbChipDataLocal = OpenDatabase(newfilename & "-Local.gmf")
      ...
      If (TreeForm.LocalPath <> rstemp2!LocalFOlder) Or _
         ...
         'need to rebuild
         lblProgress.Caption = "Your local gmf file is out of date. MAPPFinder is now going to rebuild it."
         frmCriteria.Refresh
         DoEvents
...
Public Function CalculateGOResults(criterion As Integer) As Boolean
   ...
   On Error GoTo error
   Dim rsFilter As Recordset
   Dim metFilter As Long 'no GeneOntology available
...
Public Function CalculateLocalResults(criterion As Integer) As Boolean
   On Error GoTo error
   Dim noLocalMAPP As Long
   Dim nolocalClusterC As Long
   Dim localgene As Long
...
MousePointer = vbHourglass
   ...
   Dim genes() As New PrimaryGene
   Dim GOarray() As New goterm
   Randomize 'initialzes random number generator
   ReDim genes(PrimaryGenes.count) As New PrimaryGene
   ReDim GOarray(GOterms.count) As New goterm
   Dim genecount As Double
   Dim GOvisited() As goterm 'keep track of the goterm that were visited by a ID
   ReDim GOvisited(GOterms.count) As goterm 'keep track of the goterm that were visited by a ID
   
   Dim GOvisitedcount As Long
   Dim tempGO As goterm
   totalGenes = 0
   GOvisitedcount = 0
  ...
  For Each cg In ClusterGenes
      If (Not exists(cg.getMasterPrimary, primaryIDs)) And cg.getNestedGOterms.count > 0 Then
         primaryIDs.Add cg.getMasterPrimary, cg.getMasterPrimary
         Set genes(i) = PrimaryGenes.Item(cg.getMasterPrimary)
         i = i + 1
      End If
   Next cg
   cgcount = i
   ...
   For i = 0 To TRIALS - 1 'bootstrap x trials
      ...
      genesort genes(), 0, bigN - 1 'cgcount - 1 'sort the genes by the random number between 0..1
      
      
      'now just take the first bigR as the "changed" genes
      For j = 0 To bigR - 1
         Set cgs = genes(j).getClusterGenes
         GOvisitedcount = 0
         For Each c In cgs
            Set GOIDs = c.getNestedGOterms
            For Each term In GOIDs
               'Debug.Print GOterms.count
               If term.addOne Then
                  Set GOvisited(GOvisitedcount) = term
                  GOvisitedcount = GOvisitedcount + 1
               End If
            Next term
         Next c
         'so we reset the GO terms. Make sure that each primaryID is only allowed to be
         'added once to a go term
         For k = 0 To GOvisitedcount - 1
            GOvisited(k).unvisit
         Next k
      Next j
...
Public Function exists(id As String, idlist As Collection) As Boolean
On Error GoTo error
   Dim test As String
...
MousePointer = vbHourglass
   ...
   For i = 0 To TRIALS - 1 'bootstrap x trials
      ...
      genesortlocal genes(), 0, localN - 1 'sort the genes by the random number between 0..1
...
End Sub
Private Sub genesort(tester() As PrimaryGene, start As Long, finish As Long)
    'Input - an array of strings
   'Output - the array sorted alphabetically. The sort is a mergesort (NlogN).
   'The sort is not case sensitive (ie A = a).
   Dim temp As PrimaryGene
   Dim split As Long
   Dim counter1 As Long, counter2 As Long
   Dim temparray() As PrimaryGene
   Dim i As Long
   ...
   ElseIf finish - start = 1 Then 'there are two left
      ...
      'End Sub
   Else ' need to partition and then merge
      ReDim temparray(finish - start + 1) As PrimaryGene
      split = (finish + start) / 2
...
End Sub
Private Sub genesortlocal(tester() As ClusterGene, start As Long, finish As Long)
    'Input - an array of strings
   'Output - the array sorted alphabetically. The sort is a mergesort (NlogN).
   'The sort is not case sensitive (ie A = a).
   ...
   Else ' need to partition and then merge
      ...
      split = (finish + start) / 2
      genesortlocal tester, start, split
      genesortlocal tester, split + 1, finish
      counter2 = split + 1
      counter1 = start
      i = 0
      While counter1 <= split And counter2 <= finish
