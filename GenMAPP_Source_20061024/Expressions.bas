...
Sub ExpressionDisplay(obj As Object, dbGene As Database, dbExpression As Database, _
   ...
   '     colorIndexes() The numbers of the value columns in the Display table for active Color Sets
   '                    colorIndexes(0) is the number of ColorSets in use.
...
'If obj.title = "G gamma 4" Then Stop
      For colorSet = 1 To colorIndexes(0) '=======================================For Each ColorSet
         ...
         If colorIndexes(colorSet) = valueIndex Then '==================================Apply Value
            ...
            '  This routine only operates when the value displayed is also one of the Color Sets
            '  chosen. Otherwise, the longer routine for a separate display value (following)
            '  is used.
      ...
      If valueIndex <> -1 And obj.value = "" Then
         '===========================================Apply Value If Value Not From Chosen Color Set
         colorSet = valueIndex
         lastGeneSet = 0
         GeneSets(0).systemCode = AllGenes(0).systemCode '--------------------First Gene In geneSet
         GeneSets(0).value = AllGenes(0).value
