...
Public mruColorSet As String                                                         'MRU Color Set
   '  Form: DisplayValue\ColorSet\ColorSet\.....
   '  See above
...
Sub WriteConfig() '************************************************* Rewrite The Configuration File
   ...
   '+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ Open Old And New Config Files
   Open appPath & CFG_FILE For Input As #31
   Open appPath & "config.$tm" For Output As #F_TEMP
   
   '+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ Write All GenMAPP Stuff To New File
   Print #F_TEMP, "Do not change this file, serious GenMAPP errors may occur."
   Print #F_TEMP, "ProgramVersion: " & BUILD
   Print #F_TEMP, "baseFolder: " & cfgBaseFolder
   Print #F_TEMP, "mruGeneDB: " & mruGeneDB
   mruMappPath = GetFolder(mruMappPath)                                   'Always write just folder
   Print #F_TEMP, "mruMAPPPath: " & mruMappPath
   Print #F_TEMP, "mruDataSet: " & mruDataSet
   Print #F_TEMP, "mruColorSet: " & mruColorSet
   Print #F_TEMP, "mruCatalog: " & mruCatalog
   Print #F_TEMP, "mruExportPath: " & mruExportPath
   Print #F_TEMP, "mruExportSourcePath: " & mruExportSourcePath
   Print #F_TEMP, "mruImportPath: " & mruImportPath
   Print #F_TEMP, "mruMappConvertSource: " & mruMappConvertSource
   Print #F_TEMP, "mruEDConvertSource: " & mruEDConvertSource
   Print #F_TEMP, "Coloring: " & cfgColoring
   Print #F_TEMP, "Legend: " & cfgLegend
   Print #F_TEMP, "Options: " & cfgOptions
   Print #F_TEMP, "JPEGQuality: " & cfgJPEGQuality
   Print #F_TEMP, "CheckForUpdatesOnStart: " & cfgCheckForUpdatesOnStart
   If cfgLegendAllColorSets Then
      Print #F_TEMP, "LegendColorSets: All"
   Else
      Print #F_TEMP, "LegendColorSets: First"
   End If
   Print #F_TEMP, "InitialRun: False"
   
   Do Until EOF(31) '+++++++++++++++++++++++++++++++++++ Pick Up Anything Else And Copy To New File
      Line Input #31, Lin
      If UCase(Left(Trim(Lin), 13)) = "DO NOT CHANGE" Then                        'Ignore all these
      ElseIf UCase(Left(Trim(Lin), 15)) = "PROGRAMVERSION:" Then
      ElseIf UCase(Left(Trim(Lin), 11)) = "BASEFOLDER:" Then
      ElseIf UCase(Left(Trim(Lin), 10)) = "MRUGENEDB:" Then
      ElseIf UCase(Left(Trim(Lin), 12)) = "MRUMAPPPATH:" Then
      ElseIf UCase(Left(Trim(Lin), 11)) = "MRUDATASET:" Then
      ElseIf UCase(Left(Trim(Lin), 12)) = "MRUCOLORSET:" Then
      ElseIf UCase(Left(Trim(Lin), 11)) = "MRUCATALOG:" Then
      ElseIf UCase(Left(Trim(Lin), 14)) = "MRUEXPORTPATH:" Then
      ElseIf UCase(Left(Trim(Lin), 20)) = "MRUEXPORTSOURCEPATH:" Then
      ElseIf UCase(Left(Trim(Lin), 14)) = "MRUIMPORTPATH:" Then
      ElseIf UCase(Left(Trim(Lin), 21)) = "MRUMAPPCONVERTSOURCE:" Then
      ElseIf UCase(Left(Trim(Lin), 19)) = "MRUEDCONVERTSOURCE:" Then
      ElseIf UCase(Left(Trim(Lin), 9)) = "COLORING:" Then
      ElseIf UCase(Left(Trim(Lin), 7)) = "LEGEND:" Then
      ElseIf UCase(Left(Trim(Lin), 8)) = "OPTIONS:" Then
      ElseIf UCase(Left(Trim(Lin), 12)) = "JPEGQUALITY:" Then
      ElseIf UCase(Left(Trim(Lin), 23)) = "CHECKFORUPDATESONSTART:" Then
      ElseIf UCase(Left(Trim(Lin), 16)) = "LEGENDCOLORSETS:" Then
      ElseIf UCase(Left(Trim(Lin), 11)) = "INITIALRUN:" Then
      Else                                                                   'Unrecognized, copy it
         Print #F_TEMP, Lin
      End If
   Loop
   Close #31, #F_TEMP
   Kill appPath & CFG_FILE
   Name appPath & "config.$tm" As appPath & CFG_FILE
